...
register_gh_repo_runner_usage() {
  cat <<'USAGE'
Usage: register_gh_repo_runner <hostname>

Registers a self-hosted GitHub Actions runner on the remote host for the
repository associated with the current cloned git directory.

Requirements (local):
- gh authenticated for the target repo
- ssh access to <hostname> (with sudo/root privileges on remote)
USAGE
}

register_gh_repo_runner() {
  if [[ $# -ne 1 ]]; then
    register_gh_repo_runner_usage
    return 1
  fi
  ...
  if [[ "$(git rev-parse --is-inside-work-tree 2>/dev/null || true)" != "true" ]]; then
    echo "Run this from inside a cloned git repository." >&2
    return 1
  fi

  repo_slug="$(gh repo view --json nameWithOwner --jq .nameWithOwner 2>/dev/null || true)"
  if [[ -z "$repo_slug" ]]; then
    echo "Unable to determine repo from current directory via gh." >&2
    echo "Make sure this is a cloned GitHub repo and gh is authenticated." >&2
    return 1
  fi
  ...
  runner_token="$(gh api -X POST "repos/${repo_slug}/actions/runners/registration-token" --jq .token 2>/dev/null || true)"
  if [[ -z "$runner_token" ]]; then
    echo "Failed to create registration token. Check repo admin permissions." >&2
    return 1
  fi

  echo "Connecting to ${target_host} and installing runner..."

  ssh "$target_host" \
    REPO_URL="$repo_url" \
    RUNNER_TOKEN="$runner_token" \
    HOST_LABEL="$target_host" \
    REPO_SLUG="$repo_slug" \
    'bash -s' <<'REMOTE'
set -euo pipefail
...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "Only Linux hosts are supported by this script." >&2
  exit 1
fi

if [[ "$(id -u)" -ne 0 ]] && ! command -v sudo >/dev/null 2>&1; then
  echo "Remote user must be root or have sudo privileges." >&2
  exit 1
fi

if command -v apt-get >/dev/null 2>&1; then
  as_root "apt-get update -y && apt-get install -y curl tar jq"
elif command -v dnf >/dev/null 2>&1; then
  as_root "dnf install -y curl tar jq"
elif command -v yum >/dev/null 2>&1; then
  as_root "yum install -y curl tar jq"
else
  for dep in curl tar jq; do
    if ! command -v "$dep" >/dev/null 2>&1; then
      echo "Missing dependency on remote host: $dep" >&2
      echo "Install curl, tar, and jq then re-run." >&2
      exit 1
    fi
  done
fi
...
repo_safe="${REPO_SLUG//\//-}"
runner_dir="${BASE_DIR}/${repo_safe}"
labels="${HOST_LABEL},${runner_arch},${os_label}"

latest_json="$(curl -fsSL https://api.github.com/repos/actions/runner/releases/latest)"
runner_version="$(printf '%s' "$latest_json" | jq -r '.tag_name | sub("^v"; "")')"
asset_name="actions-runner-linux-${runner_arch}-${runner_version}.tar.gz"
download_url="$(printf '%s' "$latest_json" | jq -r --arg name "$asset_name" '.assets[] | select(.name == $name) | .browser_download_url')"

if [[ -z "$download_url" || "$download_url" == "null" ]]; then
  echo "Could not resolve latest runner asset: $asset_name" >&2
  exit 1
fi

if ! id -u "$RUNNER_USER" >/dev/null 2>&1; then
  as_root "useradd --system --create-home --shell /bin/bash $RUNNER_USER"
fi

as_root "mkdir -p '$runner_dir' && chown -R '$RUNNER_USER:$RUNNER_USER' '$runner_dir'"

as_runner "cd '$runner_dir' && curl -fsSL -o runner.tar.gz '$download_url' && tar xzf runner.tar.gz"

if [[ -x "$runner_dir/bin/installdependencies.sh" ]]; then
  as_root "bash '$runner_dir/bin/installdependencies.sh'"
fi

as_runner "cd '$runner_dir' && ./config.sh --url '$REPO_URL' --token '$RUNNER_TOKEN' --name '$runner_name' --labels '$labels' --unattended --replace --work '_work'"

as_root "bash '$runner_dir/svc.sh' install '$RUNNER_USER' || true"
as_root "bash '$runner_dir/svc.sh' start"
